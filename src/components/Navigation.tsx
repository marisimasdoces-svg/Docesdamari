...
import { exportDatabaseJSON, importDatabaseJSON, resetToInitialData } from '../lib/storage';
    ...
    <>
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          >
            <div className="w-11 h-11 rounded-2xl overflow-hidden border border-amber-300 shadow-sm group-hover:scale-105 transition-all duration-200 shrink-0 bg-purple-950">
              <img
                src={appIconImg}
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="font-brand text-2xl sm:text-3xl text-gold-shimmer font-bold tracking-wide group-hover:scale-105 transition-transform leading-none flex items-center gap-1.5 drop-shadow-xs">
                  <span>Mari Simas Doces</span>
                </h1>
              </div>
              <p className="text-[11px] text-slate-500 hidden sm:block font-medium">
                Gestão de Vendas & Fechamento
              </p>
            </div>
          </button>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* View App Icon Modal Button */}
            <button
              type="button"
              onClick={() => setShowIconModal(true)}
              className="px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs hover:scale-105 active:scale-95"
              title="Visualizar Ícone do Aplicativo"
            >
              <Smartphone className="w-3.5 h-3.5 text-amber-700" />
              <span className="hidden sm:inline">Ícone App</span>
            </button>
